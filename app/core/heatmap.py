...
class HeatmapProcessor:
    ...
    def update_heatmap(self, frame: np.ndarray, detections: list) -> None:
        ...
        for box in detections:
            ...
            # ИСПОЛЬЗУЕМ НОВЫЙ ПАРАМЕТР ИНТЕНСИВНОСТИ
            cv2.circle(new_heat, (center_x, center_y),
                      heatmap_config.GAUSSIAN_RADIUS, 
                      heatmap_config.INTENSITY_PER_DETECTION, -1)
    ...
    def apply_heatmap_overlay(self, frame: np.ndarray) -> np.ndarray:
        ...
        if heatmap_config.HEATMAP_SCALING == "absolute":
            # Абсолютное масштабирование
            scaled_heatmap = np.clip(self.heatmap, 0, heatmap_config.ABSOLUTE_SCALE_MAX)
            heatmap_norm = (scaled_heatmap / heatmap_config.ABSOLUTE_SCALE_MAX * 255).astype(np.uint8)
        else:
            # Относительное масштабирование (старое поведение)
            max_value = max(np.max(self.heatmap), 1)
            heatmap_norm = (self.heatmap / max_value * 255).astype(np.uint8)

        heatmap_colored = cv2.applyColorMap(heatmap_norm, cv2.COLORMAP_JET)
        overlay = cv2.addWeighted(frame, 0.7, heatmap_colored, heatmap_config.OVERLAY_ALPHA, 0)

        # Добавляем информацию о режиме масштабирования
        elapsed_time = datetime.datetime.now() - self.heatmap_start_time
        total_heat = np.sum(self.heatmap)
        
        scaling_info = f"Mode:{heatmap_config.HEATMAP_SCALING}"
        if heatmap_config.HEATMAP_SCALING == "absolute":
            scaling_info += f" Max:{heatmap_config.ABSOLUTE_SCALE_MAX}"

        info_text = (f"Time: {elapsed_time.seconds//3600:02d}:"
                    f"{(elapsed_time.seconds%3600)//60:02d} | "
                    f"Activity: {total_heat:.0f} | {scaling_info}")
    ...
    def get_statistics(self) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            # ДОБАВИТЬ ИНФОРМАЦИЮ О НАСТРОЙКАХ
            'intensity_per_detection': heatmap_config.INTENSITY_PER_DETECTION,
            'heatmap_scaling': heatmap_config.HEATMAP_SCALING,
            'absolute_scale_max': heatmap_config.ABSOLUTE_SCALE_MAX
        }
